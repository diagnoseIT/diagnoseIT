...
public class NPlusOneRule {
	...
	public boolean action() {
		log.info("Executing NPlusOneRule..");
		...
		if (result) {
			log.severe("NPlusOneRule: N+1 anti-pattern detected");
		} else {
			log.info("NPlusOneRule: no N+1");
		}
	...
	public boolean isNPlusOneProblem(List<DatabaseInvocation> causeInvocations) {
		...
		if (amountOfNQueries > NUMBER_OF_CALLS_THRESHOLD) {
			log.info("NPlusOneRule: Amount of 'N' queries= " + amountOfNQueries);
			return true;
		}
		return false;
	}
