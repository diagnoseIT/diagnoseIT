...
public class TheStifleRule {
	...
	public boolean action() {
		log.info("Executing TheStifleRule..");
		if (resultOfNPlusOneRule == true) {
			log.info("TheStifleRule: N+1 detected, so there is also the Stifle");
			return true;
		}
		...
		if (result) {
			log.severe("TheStifleRule: The Stifle detected");
		} else {
			log.info("TheStifleRule: No Stifle");
		}
	...
	public boolean checkDatabaseProblem() {
		...
		NestingCallable parent = cause.getCallables().get(0).getParent().getParent();
	...
	public boolean isStifleProblem(List<? extends TimedCallable> causeInvocations) {
