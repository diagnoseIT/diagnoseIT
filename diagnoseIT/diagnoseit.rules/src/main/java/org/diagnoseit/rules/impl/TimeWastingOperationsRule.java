...
public class TimeWastingOperationsRule {
	...
	public List<AbstractAggregatedTimedCallable<? extends TimedCallable>> action() {
		log.info("Executing TimeWastingOperationsRule.."
				+ globalContext.getContainingSubTrace().getContainingTrace().getTraceId());

		// Create a list and add global context with its children. Then add
		// their children etc.
		List<TimedCallable> processingList = TraceUtils.asList(globalContext, TimedCallable.class);
		...
		for (AbstractAggregatedTimedCallable<? extends TimedCallable> aggCallable : aggregatedCallables) {
			...
			if ((globalContext.getResponseTime() / 1000000) - sumExecTime > baseline
					|| sumExecTime < RT_RALATION_THRESHOLD * (globalContext.getResponseTime() / 1000000)) {
		...
		log.info("Ended time-wasting operations rule. Returned " + timeWastingOperations.size() + " operation(s) in trace "
				+ globalContext.getContainingSubTrace().getContainingTrace().getTraceId());
	...
	private void sortAggCallablesDescending(
			List<AbstractAggregatedTimedCallable<? extends TimedCallable>> aggregatedCallables) {
		aggregatedCallables.sort(new Comparator<AbstractAggregatedTimedCallable<? extends TimedCallable>>() {

			@Override
			public int compare(AbstractAggregatedTimedCallable<? extends TimedCallable> o1,
					AbstractAggregatedTimedCallable<? extends TimedCallable> o2) {
				// sort descending!
				long diff = o2.getExclusiveTimeStats().getSum() - o1.getExclusiveTimeStats().getSum();
				if (diff < 0L) {
					return -1;
				} else if (diff > 0L) {
					return 1;
