...
public class InfluxDBConnector {
	 ...
	 */
	public void connect() {
		influxDB = InfluxDBFactory.connect(DATABASE_URL, DATABASE_USERNAME, DATABASE_PASSWORD);
	}
	...
	public void readData() {
		...
		Query query = new Query("SELECT * FROM " + TABLE_TIMING_INFORMATION, DATABASE_NAME);
		...
		for (Result result : qResult.getResults()) {
			for (Series serie : result.getSeries()) {
				for (List<Object> listTuples : serie.getValues()) {
					...
					DateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSXXX");
					DateFormat dateFormat2 = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SXXX");
					DateFormat dateFormat3 = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:SSSXXX");
					long timestamp = 0;
					try {
						timestamp = dateFormat.parse((String) listTuples.get(0)).getTime();
					} catch (ParseException e) {
						// TODO Auto-generated catch block
						try {
							timestamp = dateFormat1.parse((String) listTuples.get(0)).getTime();
						} catch (ParseException e1) {
							// TODO Auto-generated catch block
							try {
								timestamp = dateFormat2.parse((String) listTuples.get(0)).getTime();
							} catch (ParseException e2) {
								// TODO Auto-generated catch block
								try {
									timestamp = dateFormat3.parse((String) listTuples.get(0)).getTime();
					...
					point = new DataPointTimestamp((double) timestamp, (double) listTuples.get(1));
	...
	public List<DataPointTraceID> getDataPointsWithTraceID() {
		...
		for (Result result : qResult.getResults()) {
			for (Series serie : result.getSeries()) {
				for (List<Object> listTuples : serie.getValues()) {
					point = new DataPointTraceID((String) listTuples.get(1), (double) listTuples.get(2),
	...
	public List<DataPointTwoMeasurements> getDataPointsResponseAndCPUTimes() {
		...
		for (Result result : qResult.getResults()) {
			for (Series serie : result.getSeries()) {
				for (List<Object> listTuples : serie.getValues()) {
					point = new DataPointTwoMeasurements((double) listTuples.get(1), (double) listTuples.get(2));
					dataPoints.add(point);
				}
			}
		}
	...
	public List<String> getTraceIds() {
		...
		Query query = new Query("SHOW TAG VALUES FROM " + TABLE_TIMING_INFORMATION + " WITH KEY = TraceID",
				DATABASE_NAME);
		QueryResult qResult = influxDB.query(query);
	 ...
	 */
	public void writeData(double responseTime, double cpuTime, double gcTime, long traceId, long traceTimestamp) {

		Point point1 = Point.measurement(TABLE_TIMING_INFORMATION).tag(INFLUXDB_TAG_TRACE, String.valueOf(traceId))
				.time(traceTimestamp, TimeUnit.MILLISECONDS).addField(COLUMN_RESPONSE_TIME, responseTime)
				.addField(COLUMN_CPU_TIME, cpuTime).addField(COLUMN_GC_TIME, gcTime).build();
	...
	private static class TimeseriesThread extends Thread {
		...
		public void run() {
			...
			double responseTime1 = 13000, responseTime2=15000, responseTime3 = 11000;
			double gcTime = 0;
			double cpuTime = 5000;
			long traceId = 1;
			...
			FileWriter fw;
			try {
				// File file = new File("/home/dusan/Projects/diagnoseIT/traces/ts" + format.format(new Date()) + ".csv");
				File file = new File("/home/dusan/Projects/diagnoseIT/traces/ramp" + format.format(new Date()) + ".csv");
...
//						responseTime = ThreadLocalRandom.current().nextInt(50000, 75000);
//					} else {
//						gcTime = ThreadLocalRandom.current().nextInt(500, 3000);
//						responseTime = ThreadLocalRandom.current().nextInt(3000, 5000);
//					}
					// test.writeData(responseTime, cpuTime, gcTime, traceId,
					// System.currentTimeMillis());
					
					// ramp
					// responseTime1 += ThreadLocalRandom.current().nextInt(100, 1000);
					
					responseTime1 += ThreadLocalRandom.current().nextInt(100, 1000);
					responseTime2 = ThreadLocalRandom.current().nextInt(11000, 17000);
					responseTime3 += ThreadLocalRandom.current().nextInt(-100, 500);

					fw.append(responseTime1 + ",");
					fw.append(responseTime2 + ",");
					fw.append(responseTime3 + ",");
					// fw.append(cpuTime + ",");
					// fw.append(gcTime + ",");
					fw.append(traceId + ",");
	...
	public class DataPointTraceID {
		...
		DataPointTraceID(String traceId, double measurement1, double measurement2) {
			this.traceId = traceId;
			this.measurement1 = measurement1;
