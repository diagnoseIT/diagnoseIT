...
public class ExpensiveComputationRule {
	...
	public TimedCallable action() {
		System.out.println("ExpensiveComputationRule wird gefeuert!");
		...
		if (traceResponseTime >= baseline) {
			...
			if (methodInvocations.size() >= 1) {
				...
				if (CPUPercentage > CPU_PROCENT_REFERENCE_VALUE) {
					System.out.println("There is an expensive computation"
		...
		System.out
				.println("Trace response time is to low or there is no expensive computation");
		return null;
	}
}
